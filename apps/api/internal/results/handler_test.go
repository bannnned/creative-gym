...
func TestGetRoomResultReturnsRankedWorks(t *testing.T) {
		...
		getRoomResult: func(context.Context, string, string) (RoomResult, error) {
			return RoomResult{
				...
				RankedSubmissions: []SubmissionResult{
					{ID: "winner", Rank: 1, Wins: 8, Comparisons: 10},
	...
	if !strings.Contains(body, `"rank":1`) || !strings.Contains(body, `"is_current_user":true`) {
		t.Fatalf("body = %q, want rankings and current user", body)
	}
}
